...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🎬 ECNTV - Starting Multi-Playlist Validation");
    println!("=============================================");
    println!("📊 Playlists to process: {}", playlists.len());
    ...
    for mut playlist in playlists {
        ...
        match fetch_and_parse_playlist(&client, &playlist.url).await {
            Ok(entries) => {
                ...
                println!("📥 Loaded {} streams, {} total entries", original_count, entries.len());
                
                // Filter out unwanted streams before validation
                let (filtered_entries, filtered_count) = filter_unwanted_entries(entries);
                
                if filtered_count > 0 {
                    println!("🚫 Filtered out {} streams (adult/MP4/cinehub)", filtered_count);
                }
                ...
                println!("🔬 Validating {} streams...", streams_to_validate.len());
                ...
                println!("✅ {}: {}/{} streams valid", playlist.name, valid_count, streams_to_validate.len());
                
                all_playlists.push(playlist);
            }
            Err(e) => {
                println!("❌ Failed to process {}: {}", playlist.name, e);
            }
        }
    }
        ...
        .map(|(batch_index, (original_index, url))| {
            ...
            async move {
                ...
                if validation_result.is_valid {
                    ...
                    println!("   ✅ {} {} - {} ({})", progress, channel_name, "LIVE", speed);
                } else {
...
fn write_enhanced_playlist(
    ...
    for playlist in playlists {
        if playlist.valid_count > 0 {
            writeln!(writer, "#EXTINF:-1,🎯 {} ⚡ [{} valid / {} total]", 
...
fn print_summary(playlists: &[Playlist], total_valid: usize, total_checked: usize) {
    println!("\n=============================================");
    println!("🎯 ECNTV - VALIDATION COMPLETE");
    println!("=============================================");
